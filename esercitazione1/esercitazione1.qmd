...
## Regressione lineare semplice: salario e istruzione
...
In popolazione implica che $E(wage_i|educ_i)= \beta_0 + \beta_1 educ_i$. Assumendo omoschedasticità $Var(u_i|educ_i)= \sigma^2$  è equivalente, in popolazione, ad assumere $Var(wage_i|educ_i) = \sigma^2$.
...
### Cambio di unità di misura e interpretazione dei coefficienti:
...
```{r}
modelsummary(reg_female, gof_omit = "AIC|BIC|RMSE|R2 Adj.")
```
...
modelsummary(list("Female = 1" = reg_female, "Male = 1" = reg_male), gof_omit = "AIC|BIC|RMSE|R2 Adj.", title = "Nota: La variabile dipendente è `wage`")
```
