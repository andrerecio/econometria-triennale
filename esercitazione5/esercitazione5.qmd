...
In questa esercitazione vedremo come interpretare i coefficienti di interazione tra variabili dummy, tra una variabile dummy e una variabile continua e tra due variabili continue. L'interazione tra variabili consente di modellare situazioni in cui l'effetto di una variabile dipende dal livello di un'altra variabile.



Riprendiamo un esercizio precedente in cui abbiamo intereptato i coefficienti di varibaili dummy con categorie multiplie. Abbiamo creato 3 dummy (donne sposate, uomini sposati e donne single, cioè non sposate) e interpetato i coefficienti rispetto agli uomini single (non sposati), la nostra categoria omessa:
...
1. L'intercetta rappresenta il salario orario medio in dollari della categoria omessa (**uomini single**), cioè il valore atteso di $wage$ quando $marrmale$, $marrfemale$ e $singfem$ sono uguali a zero
...
::: {.callout-attention title="Specificare le interazioni con `fixest`"}
...
### Esempio wage and educ

Riprendiamo gli stessi passaggi dell'Esercitazione 4 sull'interazione tra due variabili continue usando i dati in `wage1`.
